...
function encodeCompactConfig(draft: ConfigDraft): string | null {
  ...
  const writeEnum = (value: string, values: readonly string[], width: 1 | 2 = 1) => {
    const index = values.indexOf(value)
    if (index < 0) throw new RangeError('compact config enum is unknown')
    bytes.push(index & 0xff)
    if (width === 2) bytes.push(index >> 8)
  }
  ...
  if (draft.systemLocale !== undefined)
    include(9, () => writeEnum(draft.systemLocale!, COMPACT_ENUMS.locale, 2))
...
function decodeCompactConfig(value: string): URLSearchParams | null {
  ...
  if (!bytes || bytes.length < 4 || bytes[0] !== 3) return null
  ...
  const readEnum = (values: readonly string[], width: 1 | 2 = 1) => {
    const low = bytes[cursor++]
    const high = width === 2 ? bytes[cursor++] : 0
    return low === undefined || high === undefined ? null : (values[low | (high << 8)] ?? null)
  }
  ...
  const valid =
    ...
    read(9, 'locale', () => readEnum(COMPACT_ENUMS.locale, 2)) &&
