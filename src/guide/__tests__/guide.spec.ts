...
describe('renderGuide', () => {
  ...
  it('renders reflector, graphics, and each desktop path without changing storage', () => {
    ...
    expect(gnome).toContain('XMODIFIERS=@im=fcitx')
    expect(gnome).not.toContain('QT_IM_MODULE=fcitx')
    expect(gnome).not.toContain('QT_IM_MODULES')
    ...
    expect(kde).not.toContain('QT_IM_MODULES')
    ...
    expect(hyprland).not.toContain('SDL_IM_MODULE=fcitx')
